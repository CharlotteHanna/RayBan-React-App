import Button from "../components/Button";
import { useState } from "react";
import {arrowRight} from "../assets/icons";
import {glasses, statistics } from "../constants";
import { bigGlasses1 } from "../assets/images";
import  GlassCard  from "../components/GlassCard";

const Hero = () => {

  const [bigGlassImg, setBigGlassImg] = useState(bigGlasses1);

  return (
    <section id="home" className="w-full flex xl:flex-row flex-col justify-center min-h-screen
    gap-10 max-container">
      <div className="relative xl:w-2/5 w-full 
      flex flex-col justify-center 
      items-start  max-xl:padding-x pt-36">
        <p className="text-xl font-montserrat text-red-600">Our Summer Collection</p>
        <h1 className="mt-10 font-palanquin text-8xl max-sm:text-[72px] max-sm:leading-[82px] font-bold">
          <span className="xl:bg-white xl:whitespace-nowrap relative z-10 pr-10">The New Arrival</span>
          <br />
          <span className="text-red-600 inline-block mt-3">RayBan </span>Sunglasses
        </h1>
        <p className="text-slate-gray font-montserrat text-lg 
        leading-8 mt-6 mb-14 sm:max-w-sm
        ">Discover stylish RayBan arrivals, quality comfort, and
          innovation for your active life.
        </p>
        <Button label="Shop Now" iconURL={arrowRight}  />
        <div className="flex justify-start items-start w-full mt-20 flex-wrap gap-16">
          {statistics.map((stat) => (
            <div key={stat.label}>
              <p className="text-4xl font-palanquin font-bold">{stat.value}</p>
              <p className="leading-7 text-slate-gray font-montserrat">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="relative flex-1 flex justify-center items-center xl:min-h-screen
      max-xl:py-40 bg-primary bg-hero bg-cover bg-center" >
        <img src={bigGlassImg} alt="glasses collection" width={600} height={500}
        className="object-contain relative z-10 pl-5" />

        <div className="flex sm:gap-6 gap-4 absolute -bottom-[8%] sm:left-[10%] max-sm:px-6">
          {glasses.map((glass) => (
              <div key={glass}>
                <GlassCard 
                imgURL={glass}
                changeBigGlassImage={(glass) => setBigGlassImg(glass)}
                bigGlassImg={bigGlassImg}
                />
              </div>

          ))}
        </div>
      </div>
    </section>
  )
}

export default Hero